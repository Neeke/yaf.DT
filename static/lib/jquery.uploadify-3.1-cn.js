...
(function($, alert) {
    ...
    var handlers = {
        ...
        onDialogOpen: function() {
            ...
            this.queueData.errorMsg = '部分文件不在上传队列中:';
        ...
        onSelectError: function(file, errorCode, errorMsg) {
            ...
            if($.inArray('onSelectError', settings.overrideEvents) < 0) {
                ...
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了大小限制 (' + settings.fileSizeLimit + ').';
                    break;
                case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                    this.queueData.errorMsg += '\n文件 "' + file.name + '" 为空.';
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n文件 "' + file.name + '" 的文件类型不能上传 (' + settings.fileTypeDesc + ').';
                    break;
                }
            }
...
// uploadify默认配置
(function($) {
    var tmp = $.fn.uploadify;
    $.fn.uploadify = function(cfg) {
        var sessionId = W.util.getCookie("PHPSESSID"),
            csrfToken = $("#web_csrf_token").val();
        if (typeof cfg === "object" || !cfg) {
            ...
            if (cfg.formData) {
                cfg.formData.sessionid = sessionId;
                cfg.formData.web_csrf_token = csrfToken;
            }
            else {
                cfg.formData = {
                    sessionid: sessionId,
            ...
            tmp.call(this, $.extend(settings, cfg));
        }
        else {
